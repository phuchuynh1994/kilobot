...
uint8_t  edge_flag,stop_flag;// flag
...
void move_diagonal(uint8_t motion);
...
uint8_t find_dangerous_neighbor(neighbor_message_buffer* buffer);
///////////////MAIN FUNCTION//////////////////////////
    ...
    if(lock == 0){
      ...
      if(edge_flag == 0 ){// kilobots move normal  
        if(move_flag == 4 ) { 
          ...
          set_color(colorNum[5]);
          move_diagonal(move_flag);
         ...
         move_diagonal(transmit_msg.data[2]);
        }
...
void message_rx(message_t *m, distance_measurement_t *d){
  if(gradient_flag == 0){
   received_gradient = received_gradient_value(m,d);
    ...
    if(m->data[0] == 100 && dist < 60 && dist != 0) {
      if(m->data[1] < own_id){
        rxbuffer_push(&swarm_neighbors,m,d);
      }
      else if (m->data[1] > own_id && dist < 60){
...
uint8_t flag_to_move(){
  ...
  if(edge_flag == 0 ){
    ...
    else if(greater_id_neighbor.neighbors[greater_id_neighbor.id_of_nearsest_neighbor].gefahr_flag == 1 && distance_to_greater_id_neighbor < 45){
        return greater_id_neighbor.state_of_neighbor;
     
    }
    else if(distance_to_neighbor < 45 && state_of_neighbor != 0){

        return state_of_neighbor;    
    }
    else if(distance_to_neighbor < 60 && state_of_neighbor == 0){
        count = 0;
      return 0;
    }
...
void set_flag(){
  gefahr_flag = (find_dangerous_neighbor(&greater_id_neighbor) || find_dangerous_neighbor(&swarm_neighbors));

    if(distance_to_obstacle < 55  && edge_flag == 0){
    edge_flag = 1;
...
uint8_t state_of_neighbors(neighbor_message_buffer *buffer){
  ...
  for (int i = 0; i < buffer->N_Neighbors; ++i)
  {
    if(dist > buffer->neighbors[i].dist){
      ...
      gefahr_flag = buffer->neighbors[i].gefahr_flag;
    }
  }
  return dist;
 }
...
void process_message(neighbor_message_buffer* buffer){
  ...
  buffer->neighbors[i].gefahr_flag = gefahr_flag;
}
...
void set_color_delay(uint8_t color){
 ...
 uint8_t find_dangerous_neighbor(neighbor_message_buffer* buffer){
  for (int i = 0; i < buffer->N_Neighbors; ++i){
    if(buffer->neighbors[i].gefahr_flag == 1 && buffer->neighbors[i].dist < 55){
      return 1;
    }
  }
  return 0;
